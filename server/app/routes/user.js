...
module.exports = function(app, passport, isLoggedIn) {
    ...
    app.post('/user', isLoggedIn, function(req, res) {
	    ...
	    if (req.user.local.length > 0 && !Validator.isLength(req.body.password, constantes.SIZE_MIN_LOGIN, constantes.SIZE_MAX_LOGIN))
		return res.status(400).send({message: 'Password size must be between '+constantes.SIZE_MIN_LOGIN+' and '+constantes.SIZE_MAX_LOGIN+'.'});
	    //if (typeof req.user.local !== 'undefined' && !Tool.isValidePassword(password))
		...
		if (typeof user.local !== 'undefined')
